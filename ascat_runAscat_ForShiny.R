...
ascat.runAscat = function(ASCATobj, gamma = 0.55, pdfPlot = FALSE, y_limit = 5, circos=NA, min_ploidy=1.5, max_ploidy=5.5, min_purity=0.1, max_purity=1.05, rho_manual = NA, psi_manual = NA, img.dir=".", img.prefix="", write_segments=FALSE) {
  ...
  if (length(goodarrays)>0) {
    ...
    # collect recorded plots for all good arrays
    plots = vector("list", length(goodarrays))
    names(plots) = colnames(ASCATobj$Tumor_LogR)[goodarrays]
    for (i in 1:length(goodarrays)) {
      plots[[i]] = list(
        sunrise_plot                = res[[goodarrays[i]]]$sunrise_plot,
        nonrounded_plot             = res[[goodarrays[i]]]$nonrounded_plot,
        ascat_profile_plot          = res[[goodarrays[i]]]$ascat_profile_plot,
        aberration_reliability_plot = res[[goodarrays[i]]]$aberration_reliability_plot
      )
    }
...
runASCAT = function(lrr, baf, lrrsegmented, bafsegmented, gender, SNPpos, chromosomes, chrnames, sexchromosomes, failedqualitycheck = FALSE,
  ...
  if (nropt>0) {
    ...
    dev.new(width = 20, height = y_limit)
    ...
    nonrounded_plot <- recordPlot()
    dev.off()

    # Instead of opening a png/pdf device, open a regular (screen) device
    dev.new(width = 20, height = y_limit)
    ...
    plotAscatProfile <- recordPlot()
    dev.off()


    # plot aberration reliability
    ...
    aberration_reliability_plot <- recordPlot()
    dev.off()
    ...
    return(list(rho = rho_opt1, psi = psi_opt1, goodnessOfFit = goodnessOfFit_opt1, nonaberrant = nonaberrant,
                ...
                sunrise_plot = sunrise_plot, nonrounded_plot = nonrounded_plot,
                ascat_profile_plot = plotAscatProfile, aberration_reliability_plot = aberration_reliability_plot))
    ...
    warning("ASCAT could not find an optimal ploidy and purity value for this sample.\n")
    return(list(rho = NA, psi = NA, goodnessOfFit = NA, nonaberrant = FALSE, nA = NA, nB = NA, seg = NA, seg_raw = NA, distance_matrix = NA,
                sunrise_plot = sunrise_plot, nonrounded_plot = NULL,
                ascat_profile_plot = NULL, aberration_reliability_plot = NULL))
  }
...
diploidprobes_fixnonPAR=function(diploidprobes, SNP_data, nonPAR, seg_info) {
  ...
  FO=FO[GenomicRanges::width(GenomicRanges::pintersect(nonPAR[S4Vectors::queryHits(FO)], SEGMENTS[S4Vectors::subjectHits(FO)]))/GenomicRanges::width(SEGMENTS[S4Vectors::subjectHits(FO)])>0.5]
  if (length(FO)==0) return(diploidprobes)
